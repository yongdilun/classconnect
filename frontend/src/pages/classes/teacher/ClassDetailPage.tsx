...
import AnnouncementForm from '../../../components/classes/AnnouncementForm';
...
const ClassDetailPage: React.FC = () => {
  ...
  // Function to refresh announcements after creating a new one
  const refreshAnnouncements = async () => {
    ...
    try {
      ...
      setAnnouncementError(err.message || 'Failed to refresh announcements');
    } finally {
      setIsLoadingAnnouncements(false);
    }
  };
  ...
  const handleTabChange = (tab: 'stream' | 'assignments' | 'chat') => {
    ...
    console.log(`Tab changed to ${tab}, navigating to: /teacher/classes/${classId}/${tab === 'stream' ? '' : tab}`);
  };
