...
func testDeleteTestData() {
	...
	result := database.DB.Where("email LIKE ?", "test_%").Find(&testUsers)
	if result.Error != nil {
		fmt.Printf("Failed to find test users: %v\n", result.Error)
		return
	}

	fmt.Printf("Found %d test users to delete\n", len(testUsers))

	// For each user, delete their profiles first
	for _, user := range testUsers {
		if user.UserRole == "teacher" {
			// Delete teacher profile
			result := database.DB.Where("user_id = ?", user.UserID).Delete(&models.TeacherProfile{})
			if result.Error != nil {
				fmt.Printf("Failed to delete teacher profile for user ID %d: %v\n", user.UserID, result.Error)
			} else if result.RowsAffected > 0 {
				fmt.Printf("Deleted teacher profile for user ID %d\n", user.UserID)
			}
		} else if user.UserRole == "student" {
			// Delete student profile
			result := database.DB.Where("user_id = ?", user.UserID).Delete(&models.StudentProfile{})
			if result.Error != nil {
				fmt.Printf("Failed to delete student profile for user ID %d: %v\n", user.UserID, result.Error)
			} else if result.RowsAffected > 0 {
				fmt.Printf("Deleted student profile for user ID %d\n", user.UserID)
			}
		}
	}
